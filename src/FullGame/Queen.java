...
public class Queen {
	...
	public static void hint(JButton[][] field, ChessMap map, int row, int col) {
		...
		int[] offs = {-1,-1,0,-1,1,0,1,1};
		
		for (int i=0;i<8;i++) span(field, map, row+offs[i], col+offs[(i+1)%8], map.getMap()[row][col],offs[i],offs[(i+1)%8],fromR,fromC);
	}
	...
	private static void span(JButton[][] field, ChessMap map, int row, int col, char root, int offr, int offc, int fromR, int fromC) {
		if (row>=0 && row<8 && col>=0 && col<8) {
			...
			else if (root == 'q' && (map.getMap()[row][col]=='R' || map.getMap()[row][col]=='B' || map.getMap()[row][col]=='N'
	...
	private static void spanDom(ChessMap map, int row, int col, char root, int offr, int offc) {
		if (row>=0 && row<8 && col>=0 && col<8) { //the looking cell is in board
			if (root=='q' && map.getMap()[row][col]=='\0') {
